...
export class EventsGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  handleJoinConversation(@ConnectedSocket() client: Socket, @MessageBody() conversationId: string): void {
    if (typeof conversationId === 'string') client.join(`conversation:${conversationId}`);
  }

  @SubscribeMessage('conversation:leave')
  handleLeaveConversation(@ConnectedSocket() client: Socket, @MessageBody() conversationId: string): void {
    if (typeof conversationId === 'string') client.leave(`conversation:${conversationId}`);
  }
