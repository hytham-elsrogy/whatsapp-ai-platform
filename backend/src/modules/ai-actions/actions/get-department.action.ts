...
export class GetDepartmentAction implements ActionDefinition {
  name = 'getDepartment';
  description = 'Look up a department by its exact name and return its id, description and working hours.';
  inputSchema = {
    type: 'object' as const,
    properties: { name: { type: 'string', description: 'Exact department name' } },
    required: ['name'],
  };
  ...
  async handler(input: Record<string, unknown>, ctx: ActionContext): Promise<ActionResult> {
    const department = await this.departmentsService.findByName(ctx.tenantId, String(input.name));
