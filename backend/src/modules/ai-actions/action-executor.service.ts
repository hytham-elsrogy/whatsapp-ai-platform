...
export class ActionExecutorService {
  ...
  async execute(
    ...
    await this.actionRepo.update(record.id, {
  ...
  private async run(
    ...
    if (!allowedActionNames.includes(actionName)) {
      this.logger.warn(
        `Action "${actionName}" rejected — not in this AI agent's whitelist`,
      );
    ...
    if (!validate(input)) {
      this.logger.warn(
        `Action "${actionName}" rejected — schema validation failed: ${ajv.errorsText(validate.errors)}`,
      );
      return {
        status: "rejected",
        output: { reason: "invalid_input", errors: validate.errors },
      };
    }
    ...
    try {
      ...
      if (error instanceof ActionNotImplementedError) {
        return {
          status: "rejected",
          output: { reason: "not_implemented", message: error.message },
        };
      }
      this.logger.error(
        `Action "${actionName}" failed: ${(error as Error).message}`,
      );
      return {
        status: "failed",
