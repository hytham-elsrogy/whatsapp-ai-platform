...
import { CLOSED_STATUSES, Conversation, ConversationStatus } from './entities/conversation.entity';
...
export class ConversationsService {
  ...
  async findAll(
    ...
    if (filter === 'mine' && currentUserId) where.assignedAgentId = currentUserId;
  ...
  async markCustomerMessageReceived(conversationId: string): Promise<void> {
    await this.repo.update(conversationId, { lastCustomerMessageAt: new Date() });
  }
  ...
  async setDepartment(conversationId: string, departmentId: string): Promise<void> {
    await this.repo.update(conversationId, { departmentId });
  }

  async setAssignedAgent(conversationId: string, agentId: string): Promise<void> {
    await this.repo.update(conversationId, { assignedAgentId: agentId });
  }

  async setPriority(conversationId: string, priority: Conversation['priority']): Promise<void> {
    await this.repo.update(conversationId, { priority });
  }

  async setAiAgent(conversationId: string, aiAgentId: string | null): Promise<void> {
