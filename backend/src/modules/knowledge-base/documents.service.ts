...
export class DocumentsService {
  ...
  findAllForKnowledgeBase(knowledgeBaseId: string): Promise<KbDocument[]> {
    return this.documentRepo.find({ where: { knowledgeBaseId }, order: { createdAt: 'DESC' } });
  }

  async findOneWithChunks(tenantId: string, id: string): Promise<DocumentWithChunks> {
  ...
  async create(tenantId: string, knowledgeBaseId: string, userId: string, dto: CreateDocumentDto) {
    ...
    try {
      ...
      this.logger.warn(`URL fetch failed for document ${document.id}: ${(error as Error).message}`);
  ...
  async createFromPdf(
    ...
    try {
      ...
      this.logger.warn(`PDF text extraction failed for document ${document.id}: ${(error as Error).message}`);
      await this.documentRepo.update(document.id, { status: 'failed' });
      return { ...document, status: 'failed' as const };
    }
    return this.ingest(document, rawText);
  ...
  private async ingest(document: KbDocument, rawText: string) {
    try {
      ...
      for (let i = 0; i < chunks.length; i++) {
        await this.embeddingsRepository.insert(chunks[i].id, vectors[i], this.embeddingProvider.modelName);
      }

      await this.documentRepo.update(document.id, { status: 'ready' });
      return { ...document, status: 'ready' as const };
    } catch (error) {
      this.logger.warn(`Document ${document.id} ingestion failed: ${(error as Error).message}`);
      await this.documentRepo.update(document.id, { status: 'failed' });
      return { ...document, status: 'failed' as const };
    }
  }
  ...
  private async fetchAndExtractUrl(url: string): Promise<string> {
    ...
    try {
      ...
      throw new BadRequestException(`Could not reach URL: ${(error as Error).message}`);
    }
    if (!response.ok) {
      throw new BadRequestException(`URL returned ${response.status} ${response.statusText}`);
    }

    const contentLength = Number(response.headers.get('content-length') ?? 0);
    if (contentLength > MAX_URL_FETCH_BYTES) {
      throw new BadRequestException(`Page too large (${contentLength} bytes) to ingest`);
    }

    const html = await response.text();
    const text = extractTextFromHtml(html);
    if (!text) throw new BadRequestException('No readable text found at that URL');
    return text;
  }
}
