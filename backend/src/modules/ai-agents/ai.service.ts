...
import { RetrieverService, RetrievedContext } from '@/modules/rag/retriever.service';
...
const FORBIDDEN_OUTPUT_PATTERNS = [/تشخيص طبي/i, /جرعة\s*\d/i, /\bmg\b.*\bdaily\b/i, /diagnos(is|e)/i];
...
export class AiService {
  ...
  async maybeStartSession(
    ...
    const session = await this.sessionRepo.save(
      this.sessionRepo.create({ conversationId: conversation.id, aiAgentId: aiAgent.id, status: 'active' }),
    );
    await this.conversationsService.setAiAgent(conversation.id, aiAgent.id);
    const aiConversation = await this.conversationsService.transition(conversation, 'ai', { type: 'system' });
  ...
  async handleMessage(conversation: Conversation, messageText: string): Promise<void> {
    const session = await this.findActiveSession(conversation.id);
    if (!session) return;

    const aiAgent = await this.aiAgentsService.findByIdUnscoped(session.aiAgentId);
    if (!aiAgent) {
  ...
  private async runTurn(
    ...
    if (matchedKeyword) {
      await this.escalate(session, conversation, `guardrail_keyword:${matchedKeyword}`, guardrails, 'urgent');
      return;
    }
    ...
    try {
      ...
      await this.escalate(session, conversation, 'ai_provider_error', guardrails);
      return;
    }
    ...
    if (intent === 'human_agent_request') {
      ...
      await this.escalate(session, conversation, 'human_agent_request', guardrails);
      return;
    }
    ...
    if (knowledgeBaseId) {
      try {
        context = await this.retrieverService.retrieve(knowledgeBaseId, messageText);
      } catch (error) {
    ...
    try {
      ...
      generated = await this.generateWithTools(aiAgent, systemPrompt, [...history, { role: 'user', content: messageText }], actionCtx);
    } catch (error) {
    ...
    if (!generated || generated.length === 0 || generated.length > 4000) {
      await this.escalate(session, conversation, 'response_validation_failed:length', guardrails);
      return;
    }
    const forbiddenHit = FORBIDDEN_OUTPUT_PATTERNS.find((p) => p.test(generated));
    if (forbiddenHit) {
      this.logger.warn(`AI session ${session.id} generated a response matching a forbidden pattern — escalating`);
      await this.escalate(session, conversation, 'response_validation_failed:forbidden_content', guardrails);
      return;
    }
  ...
  private async detectIntent(
    ...
    const response = await this.llmProvider.complete({
      model: aiAgent.model,
      systemPrompt: 'Classify the customer message into exactly one intent with a confidence score.',
    ...
    if (!call) throw new LLMProviderError(502, 'Model did not return an intent classification');
    return { intent: String(call.input.intent), confidence: Number(call.input.confidence) };
  }
  ...
  private async generateWithTools(
    ...
    for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
      ...
      for (const call of response.toolCalls) {
        const result = await this.actionExecutor.execute(call.name, call.input, actionCtx, aiAgent.allowedActions);
        await this.logMessage(
          actionCtx.aiSessionId,
          'tool',
          JSON.stringify({ name: call.name, input: call.input, status: result.status, output: result.output }),
        );
    ...
    throw new Error(`Exceeded ${MAX_TOOL_ROUNDS} tool-use rounds without a final reply`);
  }

  private buildSystemPrompt(aiAgent: AiAgent, context: RetrievedContext[]): string {
  ...
  private async logMessage(
    ...
    await this.messageRepo.save(this.messageRepo.create({ aiSessionId, role, content, sourceRefs }));
  }

  private async sendAiText(conversation: Conversation, text: string): Promise<void> {
  ...
  private async escalate(
    ...
    if (priorityOverride) {
      await this.conversationsService.setPriority(conversation.id, priorityOverride);
    }
    ...
    if (targetDepartmentId) {
      await this.assignmentService.routeToDepartment(conversation, targetDepartmentId);
    } else {
      await this.conversationsService.transition(conversation, 'waiting', { type: 'ai' });
    }
  }
