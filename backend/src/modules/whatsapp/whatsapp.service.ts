...
export class WhatsAppService {
      ...
      for (const change of entry.changes ?? []) {
        ...
        if (change.field === "message_template_status_update") {
          const newStatus = value.event
            ? META_TEMPLATE_EVENT_TO_STATUS[value.event]
            : undefined;
          if (value.message_template_id && newStatus) {
            await this.templatesService.applyStatusUpdateFromWebhook(
        ...
        try {
          whatsappNumber =
            await this.whatsappNumbersService.findByPhoneNumberId(
              phoneNumberId,
            );
        } catch {
          this.logger.warn(
            `Ignoring webhook for unregistered phone_number_id=${phoneNumberId}`,
          );
          continue;
        }
        ...
        for (const message of value.messages ?? []) {
          ...
          const customer =
            await this.customersService.findOrCreateByWhatsappNumber(
              tenantId,
              message.from,
          ...
          await this.conversationsService.markCustomerMessageReceived(
            conversation.id,
          );
          ...
          const isOptOut =
            Boolean(messageText) &&
            this.consentsService.matchesOptOutKeyword(messageText);
          ...
          if (isOptOut) {
            ...
            await this.chatbotService.handleMessage(conversation, {
              text: messageText,
            });
            ...
            const flow = whatsappNumber.chatbotFlowId
              ? await this.chatbotFlowsService.findPublishedById(
                  tenantId,
                  whatsappNumber.chatbotFlowId,
                )
              : null;
            let handled = false;
            if (flow) {
              ...
              handled = await this.aiService.maybeStartSession(
                conversation,
                whatsappNumber.aiAgentId,
                messageText,
              );
            }
            if (!handled) {
          ...
          try {
            ...
            this.logger.warn(
              `markAsRead failed (non-fatal): ${(error as Error).message}`,
            );
          }
        }
  ...
  private async sendOptOutConfirmation(
    ...
    whatsappNumber: Awaited<
      ReturnType<WhatsappNumbersService["findByPhoneNumberId"]>
    >,
  ): Promise<void> {
    const text =
    ...
    try {
      ...
      if (error instanceof MetaApiError) {
        this.logger.warn(
          `Opt-out confirmation send failed (non-fatal): ${error.message}`,
        );
        return;
      }
      throw error;
