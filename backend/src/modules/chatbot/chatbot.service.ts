...
import { CHATBOT_DELAY_QUEUE, MAX_DELAY_SECONDS } from './chatbot-delay.constants';
...
export class ChatbotService {
  ...
  async findActiveSession(conversationId: string): Promise<ChatbotSession | null> {
  ...
  async startFlow(conversation: Conversation, flow: ChatbotFlow): Promise<void> {
    const startNode = await this.nodeRepo.findOne({ where: { flowId: flow.id, type: 'start' } });
    if (!startNode) {
    ...
    const session = await this.sessionRepo.save(
      this.sessionRepo.create({ conversationId: conversation.id, flowId: flow.id, variables: {} }),
    );
    await this.conversationsService.transition(conversation, 'bot', { type: 'bot' });

    await this.runFrom(session, startNode.id, conversation);
  }

  async handleMessage(conversation: Conversation, reply: InboundReply): Promise<void> {
    const session = await this.findActiveSession(conversation.id);
    if (!session || !session.currentNodeId) return;

    const currentNode = await this.nodeRepo.findOne({ where: { id: session.currentNodeId } });
    ...
    const variableName = (currentNode.config?.variable as string) ?? currentNode.id;
    session.variables = { ...session.variables, [variableName]: reply.text.trim() };
    await this.sessionRepo.save(session);

    await this.advance(session, currentNode.id, conversation, reply.text.trim());
  }
  ...
  async resumeAfterDelay(sessionId: string, nodeId: string): Promise<void> {
    ...
    if (!session || session.endedAt || session.currentNodeId !== nodeId) {
      this.logger.debug(`Skipping stale delay resume for session ${sessionId}, node ${nodeId}`);
      return;
    }
    ...
    if (!conversation || conversation.status !== 'bot') {
      ...
      this.logger.debug(`Skipping delay resume: conversation ${session.conversationId} is no longer bot-owned`);
      return;
    }
  ...
  private async runFrom(
    ...
    if (steps >= MAX_STEPS_PER_TURN) {
      this.logger.error(`Flow ${session.flowId} exceeded max steps per turn — aborting to avoid a loop`);
    ...
    switch (node.type) {
      ...
      case 'department': {
        ...
        await this.endSession(session);
        if (departmentId) {
          await this.assignmentService.routeToDepartment(conversation, departmentId);
        }
        return;
      }
      case 'agent': {
        const departmentId =
          (node.config?.departmentId as string | undefined) ?? conversation.departmentId;
        await this.endSession(session);
        if (departmentId) {
          await this.assignmentService.routeToDepartment(conversation, departmentId);
        } else {
          await this.conversationsService.transition(conversation, 'waiting', { type: 'bot' });
        }
        return;
      }
      case 'ai': {
        ...
        await this.endSession(session);
        const started = aiAgentId ? await this.aiService.maybeStartSession(conversation, aiAgentId) : false;
        if (!started) {
          this.logger.warn(`Node ${node.id} is type "ai" but no active AI agent is configured for this department — handing off to human`);
          await this.conversationsService.transition(conversation, 'waiting', { type: 'bot' });
        }
        return;
      }
      ...
      case 'db_query': {
        ...
        this.logger.warn(`Node ${node.id} is type "${node.type}" — not implemented, skipping`);
        await this.advance(session, node.id, conversation, undefined, steps);
        return;
      }
      case 'tag': {
        ...
        if (tagName) {
          const tag = await this.tagsService.findOrCreateByName(conversation.tenantId, tagName, 'conversation');
          await this.tagsService.attachToConversation(conversation.tenantId, conversation.id, tag.id);
        } else {
          this.logger.warn(`Node ${node.id} is type "tag" but has no config.tagName — skipping`);
        }
        await this.advance(session, node.id, conversation, undefined, steps);
        return;
      }
      case 'delay': {
        const seconds = Math.min(Number(node.config?.seconds ?? 0), MAX_DELAY_SECONDS);
        if (seconds <= 0) {
          await this.advance(session, node.id, conversation, undefined, steps);
          return;
        }
        ...
        await this.delayQueue.add(
          ...
          { delay: seconds * 1000, jobId: `chatbot-delay:${session.id}:${node.id}` },
        );
        return;
      }
      case 'end':
  ...
  private async advance(
    ...
    const edges = await this.edgeRepo.find({ where: { sourceNodeId: fromNodeId } });
    if (edges.length === 0) {
      await this.endSession(session);
      return;
    }
  ...
  private edgeMatches(edge: ChatbotEdge, replyValue: string | undefined): boolean {
  ...
  private resolveConditionValue(node: ChatbotNode, session: ChatbotSession): string | undefined {
  ...
  private getOptions(node: ChatbotNode): Array<{ id: string; title: string }> {
    return (
      (node.config?.buttons as Array<{ id: string; title: string }> | undefined) ??
      (node.config?.items as Array<{ id: string; title: string }> | undefined) ??
      []
    );
  }
  ...
  private async waitForReply(session: ChatbotSession, nodeId: string): Promise<void> {
    session.currentNodeId = nodeId;
    await this.sessionRepo.save(session);
  }

  private async endSession(session: ChatbotSession): Promise<void> {
    await this.sessionRepo.update(session.id, { endedAt: new Date(), currentNodeId: null });
  }

  private async sendBotText(conversation: Conversation, text: string): Promise<void> {
    if (!text) return;
    ...
    try {
      ...
      if (error instanceof MetaApiError) {
        this.logger.warn(`Bot message send failed (non-fatal): ${error.message}`);
        return;
      }
      throw error;
    }
   ...
   */
  private async sendBotInteractive(conversation: Conversation, node: ChatbotNode): Promise<void> {
    const text = String(node.config?.text ?? '');
    ...
    try {
      const interactive =
        node.type === 'button'
          ...
          : { type: 'list' as const, bodyText: text, buttonText: 'اختر', rows: options };
      ...
      if (error instanceof MetaApiError) {
        this.logger.warn(`Interactive message send failed (non-fatal): ${error.message}`);
        return;
      }
      ...
      this.logger.warn(`Interactive message config invalid, falling back to text: ${(error as Error).message}`);
