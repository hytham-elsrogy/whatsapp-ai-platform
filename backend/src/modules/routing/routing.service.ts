...
export class RoutingService {
  ...
  async selectAgent(
    ...
    const department =
      await this.departmentsService.findByIdUnscoped(departmentId);
    const agents =
      await this.departmentsService.getDepartmentAgents(departmentId);
    if (agents.length === 0) {
    ...
    switch (department.routingStrategy) {
      case "round_robin": {
        ...
        await this.departmentsService.advanceRoundRobinCursor(
          departmentId,
          index + 1,
        );
      ...
      case "least_active": {
        const counts = await Promise.all(
          agents.map(async (a) => ({
            userId: a.userId,
            count: await this.conversationsService.countActiveForAgent(
              a.userId,
            ),
          })),
        );
      ...
      case "skill_based": {
        ...
        const skilled = agents.filter((a) =>
          (a.skills ?? []).some((skill) => tagNames.includes(skill)),
        );
