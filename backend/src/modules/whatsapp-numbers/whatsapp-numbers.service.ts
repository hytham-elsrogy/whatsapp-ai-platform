...
export class WhatsappNumbersService {
  ...
  async findByPhoneNumberId(phoneNumberId: string): Promise<WhatsappNumber> {
    ...
    if (!number) {
      throw new NotFoundException(`No WhatsApp number registered for phone_number_id=${phoneNumberId}`);
    }
    return number;
  }

  create(tenantId: string, dto: CreateWhatsappNumberDto): Promise<WhatsappNumber> {
    const number = this.repo.create({ ...dto, tenantId });
    return this.repo.save(number);
  }

  async update(tenantId: string, id: string, dto: UpdateWhatsappNumberDto): Promise<WhatsappNumber> {
  ...
  resolveAccessToken(number: WhatsappNumber): string {
    const scoped = number.accessTokenSecretRef ? process.env[number.accessTokenSecretRef] : undefined;
