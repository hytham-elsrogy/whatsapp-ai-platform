...
export class CustomerTagsController {
  ...
  async attach(
    ...
    const resolvedTagId = tagId ?? (await this.tagsService.findOrCreateByName(user.tenantId, name!, 'customer')).id;
    await this.tagsService.attachToCustomer(customerId, resolvedTagId);
    return this.tagsService.listForCustomer(customerId);
  }

  @Delete(':tagId')
  async detach(@Param('customerId') customerId: string, @Param('tagId') tagId: string) {
