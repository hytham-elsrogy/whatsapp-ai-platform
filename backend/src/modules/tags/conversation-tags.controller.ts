...
export class ConversationTagsController {
  ...
  async attach(
    ...
    const resolvedTagId = tagId ?? (await this.tagsService.findOrCreateByName(user.tenantId, name!, 'conversation')).id;
    await this.tagsService.attachToConversation(user.tenantId, conversationId, resolvedTagId);
    return this.tagsService.listForConversation(conversationId);
  }

  @Delete(':tagId')
  async detach(@Param('conversationId') conversationId: string, @Param('tagId') tagId: string) {
