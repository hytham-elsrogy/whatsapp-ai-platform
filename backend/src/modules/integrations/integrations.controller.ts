...
export class IntegrationsController {
  ...
  @Patch(':id')
  update(@CurrentUser() user: User, @Param('id') id: string, @Body() dto: Partial<CreateIntegrationDto>) {
    return this.integrationsService.update(user.tenantId, id, dto);
  }

  @Post(':id/test')
  async testConnection(@CurrentUser() user: User, @Param('id') id: string, @Body('action') action: string) {
    const integration = await this.integrationsService.findOne(user.tenantId, id);
    return this.integrationExecutorService.testConnection(user.tenantId, integration, action, {});
  }
}
